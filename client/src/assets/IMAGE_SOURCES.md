# MaMlinzi image sources

These local assets were selected from the approved candidate sources and verified as **Free to use** on Pexels on 13 September 2026. Re-check the individual source page and current Pexels licence before any production distribution.

| Local asset | Source | Creator | Intended use |
| --- | --- | --- | --- |
| `maternal-hero.jpg` | https://www.pexels.com/photo/a-pregnant-woman-holding-her-tummy-10690896/ | directorvinny studios | Public landing hero |
| `rwanda-clinician.jpg` | https://www.pexels.com/photo/confident-female-doctor-at-work-in-rwanda-37454254/ | Musinga Pictures | About page |

Assets are locally hosted rather than hotlinked, and rendered with descriptive alt text.

## User-provided homepage images

| Local asset | Placement |
| --- | --- |
| `homepage-mother.jpeg` | Homepage hero |
| `homepage-doctor.jpeg` | Homepage care-team visual and About page |
| `maternal-consultation.jpeg` | About-page supportive maternal-care consultation |

These files were supplied directly by the project owner. Confirm their usage rights before public deployment.
