...
async function getOverview(req, res) {
  try {
    ...
    const { data: assessments, error: assessmentsError } = await supabase
      ...
      .select("id, priority, triggered_rules, recommendation, created_at")
      .order("created_at", {
        ascending: false,
      });
    ...
    const { data: followups, error: followupsError } = await supabase
      .from("followups")
      .select("id, status, action, notes, created_at")
      .order("created_at", {
        ascending: false,
      });
    ...
    assessments.forEach((assessment) => {
      const priority = String(assessment.priority || "")
        .trim()
        .toLowerCase();
    ...
    const recentActivity = [
      ...recentAssessments.map((assessment) => ({
        ...
        description: `${String(assessment.priority || "").toLowerCase() || "Routine"} priority information was recorded.`,
      ...
      ...recentFollowups.map((followup) => ({
        ...
        description: followup.action || "A follow-up was recorded in MaMlinzi.",
    ...
    return res.status(500).json({
      error: "Could not load Intelligence Hub data.",
    });
  }
}
